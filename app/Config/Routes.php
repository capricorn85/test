...
$routes->group('yjs', ['namespace' => 'Api\Controllers'], function($routes)//定义api接口路由
{
    $routes->get('/', 'YJS::index');
    $routes->get('cate', 'YJS::Category');
    $routes->get('contact', 'YJS::Contact');
    $routes->get('cate/(:alpha)', 'YJS::Category/$1');
});
$routes->get('/', 'Home::index');
//前台页面
$routes->get('cate/(:num)', 'Home::category/$1');
$routes->get('cate/detail/(:num)', 'Home::article/$1');
$routes->group('screen', ['namespace' => 'Screen\Controllers'], function($routes)//定义files接口路由
{
    $routes->get('/', 'Home::index');
