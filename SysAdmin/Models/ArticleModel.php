...
class ArticleModel extends \CodeIgniter\Model
{
    ...
    protected $allowedFields = ['nav_id','article_title','author','thumb','sort_order','update_date','creator_id','creat_date','updater_id','is_show','thumb','atime','issue_date','a_type'];
//    public $pager = \Config\Services::pager();
    ...
    public function getArticle($sarray,$id=false){
        if ($id)
        {
            return $this->select($sarray)->join("nav","nav.id=article.nav_id")->join("article_content","article_content.art_id=article.id")->where('article.id',$id)->first();
