                ...
                <div class="layui-table-tool">
                    <div class="layui-btn-container">
                        <?php if (isset($nav_mold[1])&&$nav_mold[1]==1):?>
                            <button class="layui-btn layui-btn-sm layui-btn-normal" onclick="javascript:open_window('<?=site_url("xccmssys/Swipers/form")?>','新增幻灯片')" >新增</button>
                        <?php endif;?>
        ...
        <table class="layui-table" style="margin: 0 0 10px">
            ...
            <?php foreach($list as $row): ?>
                <tr>
                    ...
                    <td ><?=$row['title']?></td>
                <td><img style="max-height: 50px" src="<?=base_url($row['href'])?>"></td>
                    <td ><?=$row['sort_order']?></td>

                    <td ><?=hp_i_type($row['itype'])?></td>
                    <td ><?=hp_show($row['is_show'])?></td>
                    <?php if (isset($nav_mold[2])||isset($nav_mold[3])):?>
                        <td>
                            <div class="layui-btn-group">
                            <?php if (isset($nav_mold[2])&&$nav_mold[2]==1):?>
                                <a class="layui-btn layui-btn-xs" lay-event="edit" href="javascript:open_window('<?=site_url("xccmssys/Swipers/form?id=".$row['id'])?>','编辑图片--<?=$row['title']?>')">修改</a>
                            <?php endif;?>
                            <?php if (isset($nav_mold[3])&&$nav_mold[3]==1):?>
...
<script>
    ...
    function del(){
        ...
        n="<?=site_url('xccmssys/Swipers/deleteSwiper')?>";
        delMsg2(x,n);
    }
