            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select name="itype"  lay-filter="itype" >
                        <?php foreach (hp_img_type() as $value): ?>
...
<script>
    layui.use(['layer', 'jquery', 'form','upload','wangEditor'], function () {
        ...
        //var contentE='<?//=isset($list['content'])?$list['content']:''?>//';
        //ue.ready(function() {
        //    // editor准备好之后才可以使用
        //    if (contentE!=""){
        //        ue.setContent(contentE);
        //    }
        //});
        ...
        var uploadInst = upload.render({
            elem: '#test1'
            ,acceptMime: 'image/*'
            ,url: '<?=site_url('upLoad/upLoadSwiperImg')?>' //改成您自己的上传接口
            ,before: function(obj){
                //预读本地文件示例，不支持ie8
        ...
        form.on('submit(formDemo)', function(data){
            ...
            layer.confirm('确定提交信息？',{btn:["确定","取消"],icon: 3, title:'提示'}, function(index){
                if(!lock) {
                    ...
                    $.post("<?=site_url('sourceSwipers/eform')?>",data.field,function(res){
