...
<div class="layuimini-container">
            ...
            <form class="layui-form" id="cateForm" method="post" >
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-inline">
                        <input type="text" name="cat_name" lay-verify="required" class="layui-input" value="<?=isset($list['cat_name'])?$list['cat_name']:''?>" placeholder="请输入栏目名称">
                    </div>
                </div>
                <div class="layui-form-item">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-form-mid layui-word-aux">排序数字范围0-999，数字越大排序越前！</div>
                            ...
                            <div class="layui-form-item">
                                <label  for="column_ab" class="layui-form-label">栏目缩写</label>
                                <div class="layui-input-inline">
                                    <input type="text" name="column_ab"  value="<?=isset($list['column_ab'])?$list['column_ab']:''?>" lay-verify="required" class="layui-input" />

                                </div>
                                <div class="layui-form-mid layui-word-aux"> 如为一级栏目，请必须填写栏目缩写，否则可能会导致栏目无法正常显示！</div>
                            </div>
                        ...
                        <?php foreach($cat_list as $row): ?>
                            <?php if($row['pid']==0): ?><!--    输出一级栏目                        -->
                                <?php if ($row['id']!=(isset($_GET['id'])?$_GET['id']:0)):?>
                                    <option value="<?=$row['id']?>" <?php if($row['id']==$pid):?> selected <?php endif;?>><?=$row['cat_name']?></option>
                                    <?php if ($clevelf==0):?>
                                        <?php foreach($cat_list as $row2): ?>
                                            <?php if($row2['pid']==$row['id']): ?><!--    输出二级栏目                        -->
                                                <?php if ($row2['id']!=(isset($_GET['id'])?$_GET['id']:0)):?>
                                                    <option value="<?=$row2['id']?>" <?php if($row2['id']==$pid):?> selected <?php endif;?>>|--<?=$row2['cat_name']?></option>
                                                <?php endif;?>
                                            <?php endif;?>
                                        <?php endforeach; ?>
                                    <?php endif;?>
                ...
                <div class="layui-form-item">
                    <label for="cat_type"  class="layui-form-label">栏目类型</label>
                    <div class="layui-input-inline">
                        <select name="cat_type"  lay-filter="cat_type" >
                            <?php foreach (hp_cate_type() as $value): ?>
                                <?php if($value["id"]!=5):?>
                                <option value="<?=$value["id"]?>" <?php if(isset($list)&&$list["cat_type"]==$value["id"]): ?>selected<?php endif; ?>><?=$value["text"]?></option>
                                <?php endif;?>
                            <?php endforeach ?>
                        </select>
                ...
                <div class="layui-form-item" id="cat_type" style="display: none;">
    ...
    <script>
        layui.use(['layer', 'jquery', 'form','upload'], function () {
            ...
            myeditor=0;
            html='';
            var cat_type=<?=isset($list["cat_type"])?$list["cat_type"]:0?>;
            if (cat_type==3){
                myeditor=1;
                $("#cat_type").show();
            ...
            form.on('select(cat_type)', function(data){
                if(data.value == 3&&myeditor==0){
                    ...
                    editorConfig.MENU_CONF['uploadImage'] = {
                        server: '<?=site_url('SysAdmin/upLoad/editorImg')?>',
                        timeout: 5 * 1000, // 5s
                        fieldName: 'file',
                        allowedFileTypes: ['image/*'],
                        // meta: { token: 'xxx', a: 100 },
            ...
            var uploadInst = upload.render({
                elem: '#test1'
                ,acceptMime: 'image/*'
                ,url: '<?=site_url('SysAdmin/upLoad/upLoadCatImg')?>' //改成您自己的上传接口
                ,before: function(obj){
                    //预读本地文件示例，不支持ie8
            ...
            form.on('submit(formDemo)', function(data){
                ...
                layer.confirm('确定提交信息？',{btn:["确定","取消"],icon: 3, title:'提示'}, function(index){
                    if(!lock) {
                        ...
                        $.post("<?=site_url('SysAdmin/cate/eform')?>",data.field,function(res){
                            // console.log(res);
