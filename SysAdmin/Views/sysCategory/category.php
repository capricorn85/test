                ...
                <div class="layui-table-tool">
                    <div class="layui-btn-container">
                        <?php if (isset($nav_mold[1])&&$nav_mold[1]==1):?>
                            <button class="layui-btn layui-btn-sm layui-btn-normal" onclick="javascript:open_window('<?=base_url("xccmssys/sysCategory/form")?>','新增栏目')" >新增</button>
                        <?php endif;?>
        ...
        <table class="layui-table" style="margin: 0 0 10px">
            ...
            <?php foreach($list as $row): ?>
                <tr>
                    ...
                    <td ><?=$row['title']?></td>
                    <td ><?=$row['sort']?></td>
                    <td ><?=hp_cate_nav($row['cate_id'])?></td>
                    <td ><?=$row['href']?></td>
                    <td >
                        <i class="<?=$row['icon']?>"></i>
                    </td>
                    <td >无</td>
                    <td ><?=hp_status($row['status'])?></td>
                    <?php if (isset($nav_mold[2])||isset($nav_mold[3])):?>
                        <td>
                            <div class="layui-btn-group">
                            <?php if (isset($nav_mold[2])&&$nav_mold[2]==1):?>
                                <a class="layui-btn layui-btn-xs" lay-event="edit" href="javascript:open_window('<?=base_url("xccmssys/sysCategory/form?id=".$row['id'])?>','编辑栏目--<?=$row['title']?>')">修改</a>
                            <?php endif;?>
                            <?php if (isset($nav_mold[3])&&$nav_mold[3]==1):?>
                ...
                <?php if(isset($row['child'])): ?>
                    <?php foreach($row['child'] as $row2): ?>
                        <tr>
                            ...
                            <td >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?=$row2['title']?></td>
                            <td ><?=$row2['sort']?></td>
                            <td ><?=hp_cate_nav($row2['cate_id'])?></td>
                            <td ><?=$row2['href']?></td>
                            <td > <i class="<?=$row2['icon']?>"></i>
                                </td>
                            <td ><?=$row['title']?></td>
                            <td ><?=hp_status($row2['status'])?></td>
                            <?php if (isset($nav_mold[2])||isset($nav_mold[3])):?>
                                <td >
                                    <div class="layui-btn-group">
                                    <?php if (isset($nav_mold[2])&&$nav_mold[2]==1):?>
                                        <a class="layui-btn layui-btn-xs" lay-event="edit" href="javascript:open_window('<?=base_url("xccmssys/sysCategory/form?id=".$row2['id'])?>','编辑栏目--<?=$row2['title']?>')">修改</a>
                                    <?php endif;?>
                                    <?php if (isset($nav_mold[3])&&$nav_mold[3]==1):?>
                        ...
                        <?php if(isset($row2['child'])): ?>
                            <?php foreach($row2['child'] as $row3): ?>
                                <tr>
                                    ...
                                    <td >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-—<?=$row3['title']?></td>
                                    <td ><?=$row3['sort']?></td>
                                    <td ><?=hp_cate_nav($row3['cate_id'])?></td>
                                    <td ><?=$row3['href']?></td>
                                    <td ><i class="<?=$row3['icon']?>"></i></td>
                                    <td ><?=$row2['title']?></td>
                                    <td ><?=hp_status($row3['status'])?></td>
                                    <?php if (isset($nav_mold[2])||isset($nav_mold[3])):?>
                                        <td >
                                            <div class="layui-btn-group">
                                            <?php if (isset($nav_mold[2])&&$nav_mold[2]==1):?>
                                                <a class="layui-btn layui-btn-xs" lay-event="edit" href="javascript:open_window('<?=base_url("xccmssys/sysCategory/form?id=".$row3['id'])?>','编辑栏目--<?=$row3['title']?>')">修改</a>
                                            <?php endif;?>
                                            <?php if (isset($nav_mold[3])&&$nav_mold[3]==1):?>
...
<script>
    ...
    <?php if (isset($nav_mold[3])&&$nav_mold[3]==1):?>
        ...
        n="<?=base_url('xccmssys/sysCategory/deleteCat')?>";
        delMsg2(x,n);
    }
