...
class Article extends BaseController
{
    ...
    public function index()
    {
        ...
        $cat_list = $NavModel->select('id,label,n_type,pid,status,sort_order')->whereIn('n_type',[2,4])->get()->getResultArray();
...
//        var_dump($cat_list);
        ...
        $data = [
            'list'  => $ArticleModel->select('article.id,article_title,author,label,article.sort_order,atime,update_date,updater_id,article.is_show')
                ->where($where)
                ->like($like)
                ->where('a_type',2)
        ...
        return view('SysAdmin\Views\Article\articleList',$data);
    }
    function condition(){
        $where = [];
        $like = [];
    ...
    function  eform(){
        ...
        if ($_POST){
            ...
            $data = $this->request->getPost(array('article_title','sort_order','is_show','nav_id','atime','author'));
    ...
    function form(){
        ...
        $NavModel = new NavModel();
        $cat_list = $NavModel->select('id,label,n_type,pid')->where('status',1)->where('n_type',2)->get()->getResultArray();
//        dd($_SESSION);

//        $cat_list = $CateModel->select('id,cat_name,cat_type,pid')->where('is_show',1)->whereIn('cat_type',[2,4])->orderBy('sort_order')->get()->getResultArray();
//        $cat_list = $this->buildMenuChild(0,$cat_list);
//        $cat_list=$cat_list['my'];
        $data['a_type']=2;
        if($id){
            $list = $ArticleModel->getArticle('article.id,article_title,author,label,article.sort_order,article.is_show,article_content.content,article.atime',$id);
            $data['list']=$list;
        }
        $data['cat_list']=$cat_list;
        return view('SysAdmin\Views\Article\art_form',$data);
    }
//递归获取子菜单
